...
└── README.md
  ...
  <ol>
    <li>
      ...
      <code>ml_training_dataset.csv</code> как декартово произведение пользователей, офферов и 
      контекста, дополненное признаками взаимодействий (clicked).
    </li>
    ...
    <li>
      ...
      ранжируются и выбирается лучший.
    </li>

    <li>
      Генерируется текстовый профиль пользователя: 
      краткое описание его поведения и предпочтений на языке человека.
    </li>
    ...
    <li>
      ...
      На выходе генерируется персональное маркетинговое сообщение в стиле push / email / SMS.
    </li>
  ...
  <ol>
    ...
    <li>
      ...
      <ul>
        <li>01: EDA и анализ данных</li>
        <li>02: формирование датасета для ML</li>
        <li>03: обучение ML-ранжировщика</li>
        <li>04: генерация текстового профиля пользователя</li>
        <li>05: генерация LLM-сообщений</li>
        <li>06: демонстрация NBO (user → best offer → LLM message)</li>
      </ul>
    </li>
  </ol>
